-- =============================================================
-- SETUP NODO CBBA (Cochabamba) - SQL Server
-- Ejecutar en SQL Server Management Studio o sqlcmd
-- sqlcmd -S localhost -U sa -P Admin1234! -i setup_cbba.sql
-- =============================================================

-- Crear base de datos
IF NOT EXISTS (SELECT name FROM sys.databases WHERE name = 'hospital_cbba')
...
-- =============================================================
-- FRAGMENTO HORIZONTAL PRIMARIO: Paciente (id_hospital = 2)
-- IDs inician en 10000 para evitar colision con LPZ (1-9999)
-- =============================================================

IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='paciente' AND xtype='U')
CREATE TABLE paciente (
    id_paciente      INT          IDENTITY(10000,1) PRIMARY KEY,
...
-- =============================================================
-- FRAGMENTO HORIZONTAL PRIMARIO: Doctor (id_hospital = 2)
-- =============================================================

IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='doctor' AND xtype='U')
CREATE TABLE doctor (
    id_doctor    INT          IDENTITY(10000,1) PRIMARY KEY,
    nombre       VARCHAR(100) NOT NULL,
    apellido     VARCHAR(100) NOT NULL,
...
CREATE TABLE consulta (
    id_consulta INT          IDENTITY(10000,1) PRIMARY KEY,
...
CREATE TABLE historial_clinico_v1 (
    id_historial          INT          IDENTITY(10000,1) PRIMARY KEY,
...
CREATE TABLE emergencia (
    id_emergencia   INT          IDENTITY(10000,1) PRIMARY KEY,
    fecha           DATE         DEFAULT CAST(GETDATE() AS DATE),
    hora            TIME         DEFAULT CAST(GETDATE() AS TIME),
...
CREATE TABLE receta (
    id_receta    INT          IDENTITY(10000,1) PRIMARY KEY,
    fecha        DATE         DEFAULT CAST(GETDATE() AS DATE),
...
-- =============================================================
-- FRAGMENTO HORIZONTAL DERIVADO: Transferencias (hospital_origen = 2)
-- =============================================================
...
CREATE TABLE transferencias_hospitalarias (
    id_transferencia    INT          IDENTITY(10000,1) PRIMARY KEY,
...
-- =============================================================
-- REPLICA PARCIAL ASINCRONA: Fragmento critico V1 de LPZ y STCZ
-- =============================================================
...
CREATE TABLE historial_replica (
    ...
    hospital_origen       VARCHAR(10)  NOT NULL,  -- 'LPZ' o 'STCZ'
    tipo_sangre           VARCHAR(5),
    alergias              NVARCHAR(MAX) DEFAULT '',
    enfermedades_cronicas NVARCHAR(MAX) DEFAULT '',
...
-- Requiere: PostgreSQL ODBC Driver 16 instalado + DSN configurado
-- =============================================================
-- Paso 1: Crear DSN del sistema en ODBC de Windows:
--   DSN Name: LPZ_POSTGRES
--   Driver: PostgreSQL Unicode(x64)
--   Server: 26.91.247.115  Port: 5432  Database: hospital_lpz
--
-- Paso 2: Ejecutar en SQL Server Management Studio:
...
SELECT * FROM OPENQUERY(LPZ_LINK,
    'SELECT p.id_paciente, p.nombre, p.apellido, v1.tipo_sangre, v1.alergias, v1.enfermedades_cronicas
...
SELECT 'Laura','Vásquez Rojas','Medicina General','60011111','lvasquez@hrcbba.bo',2
WHERE NOT EXISTS (SELECT 1 FROM doctor WHERE correo='lvasquez@hrcbba.bo');

INSERT INTO doctor (nombre, apellido, especialidad, telefono, correo, id_hospital)
SELECT 'Miguel','Torrico Paz','Traumatología','60022222','mtorrico@hrcbba.bo',2
WHERE NOT EXISTS (SELECT 1 FROM doctor WHERE correo='mtorrico@hrcbba.bo');

INSERT INTO doctor (nombre, apellido, especialidad, telefono, correo, id_hospital)
SELECT 'Sandra','Lima Arce','Ginecología','60033333','slima@hrcbba.bo',2
WHERE NOT EXISTS (SELECT 1 FROM doctor WHERE correo='slima@hrcbba.bo');
GO

INSERT INTO paciente (nombre, apellido, ci, fecha_nacimiento, sexo, direccion, telefono, tipo_sangre, alergias, id_hospital)
SELECT 'Carlos','Mamani Quispe','4567890','1982-05-10','M','Av. Heroinas 234, Cbba','61111111','B+','Polen',2
WHERE NOT EXISTS (SELECT 1 FROM paciente WHERE ci='4567890');

INSERT INTO paciente (nombre, apellido, ci, fecha_nacimiento, sexo, direccion, telefono, tipo_sangre, alergias, id_hospital)
SELECT 'Ana','Lima Torres','5678901','1995-09-18','F','Calle Ecuador 567, Cbba','62222222','AB-','Ninguna',2
WHERE NOT EXISTS (SELECT 1 FROM paciente WHERE ci='5678901');
GO
...
UNION ALL SELECT 'paciente (CBBA)',   COUNT(*) FROM paciente
UNION ALL SELECT 'doctor (CBBA)',     COUNT(*) FROM doctor;
