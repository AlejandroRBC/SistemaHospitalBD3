...
def _notificar_lpz_replica(id_paciente, tipo_sangre, alergias, enfermedades_cronicas):
    db.lpz_post('/api/replica', {
        'id_paciente': id_paciente, 'hospital_origen': 'STCZ',
...
def _registrar_en_catalogo_lpz(id_paciente):
    db.lpz_post('/api/catalogo/registro', {
        'id_paciente': id_paciente, 'nodo': 'STCZ', 'id_hospital': config.ID_HOSPITAL
    })
...
def index():
    stats = {
        'pacientes'  : (db.fetchone('SELECT COUNT(*) AS n FROM frag_paciente_stcz')  or {}).get('n', 0),
        'consultas'  : (db.fetchone('SELECT COUNT(*) AS n FROM frag_consulta_stcz')  or {}).get('n', 0),
        'emergencias': (db.fetchone('SELECT COUNT(*) AS n FROM frag_emergencia_stcz') or {}).get('n', 0),
        'doctores'   : (db.fetchone('SELECT COUNT(*) AS n FROM frag_doctor_stcz')    or {}).get('n', 0),
...
@app.route('/pacientes')
def pacientes():
    rows = db.fetchall('SELECT * FROM frag_paciente_stcz ORDER BY apellido, nombre')
...
def nuevo_paciente():
    if request.method == 'POST':
        d = request.form
        pid = db.execute(
            'INSERT INTO frag_paciente_stcz (nombre, apellido, ci, fecha_nacimiento, sexo, direccion, telefono, tipo_sangre, alergias, id_hospital) VALUES (?,?,?,?,?,?,?,?,?,?)',
            (d['nombre'], d['apellido'], d['ci'], d['fecha_nacimiento'],
             d['sexo'], d['direccion'], d['telefono'], d['tipo_sangre'],
             d['alergias'], config.ID_HOSPITAL)
        )
        db.execute('INSERT INTO historial_clinico_v1 (id_paciente, tipo_sangre, alergias, enfermedades_cronicas) VALUES (?,?,?,?)',
        ...
        if hid:
            db.execute('INSERT INTO historial_clinico_v2 (id_historial, id_paciente, fecha_apertura, antecedentes, observaciones) VALUES (?,?,GETDATE(),?,?)',
...
def paciente_detalle(id_paciente):
    p = db.fetchone('SELECT * FROM frag_paciente_stcz WHERE id_paciente = ?', (id_paciente,))
    if not p:
    ...
    consultas = db.fetchall(
        "SELECT c.*, d.nombre + ' ' + d.apellido AS medico FROM frag_consulta_stcz c JOIN frag_doctor_stcz d ON c.id_doctor = d.id_doctor WHERE c.id_paciente = ? ORDER BY c.fecha DESC",
        (id_paciente,)
    )
    emergencias = db.fetchall('SELECT * FROM frag_emergencia_stcz WHERE id_paciente = ? ORDER BY fecha DESC', (id_paciente,))
...
def actualizar_historial():
    ...
    db.execute('UPDATE historial_clinico_v1 SET tipo_sangre=?, alergias=?, enfermedades_cronicas=? WHERE id_paciente=?',
               (d['tipo_sangre'], d['alergias'], d['enfermedades_cronicas'], pid))
    db.execute('UPDATE historial_clinico_v2 SET antecedentes=?, observaciones=? WHERE id_paciente=?',
...
def consultas():
    rows = db.fetchall(
        "SELECT c.*, p.nombre + ' ' + p.apellido AS paciente, d.nombre + ' ' + d.apellido AS medico FROM frag_consulta_stcz c JOIN frag_paciente_stcz p ON c.id_paciente = p.id_paciente JOIN frag_doctor_stcz d ON c.id_doctor = d.id_doctor ORDER BY c.fecha DESC"
    )
...
def nueva_consulta():
    if request.method == 'POST':
        d = request.form
        db.execute(
            'INSERT INTO frag_consulta_stcz (fecha, hora, motivo, diagnostico, id_paciente, id_doctor, id_hospital) VALUES (?,?,?,?,?,?,?)',
    ...
    pacientes_list = db.fetchall('SELECT id_paciente, nombre, apellido FROM frag_paciente_stcz ORDER BY apellido')
    doctores_list  = db.fetchall('SELECT id_doctor, nombre, apellido, especialidad FROM frag_doctor_stcz ORDER BY apellido')
...
def emergencias():
    rows = db.fetchall(
        "SELECT e.*, p.nombre + ' ' + p.apellido AS paciente FROM frag_emergencia_stcz e JOIN frag_paciente_stcz p ON e.id_paciente = p.id_paciente ORDER BY e.fecha DESC"
    )
...
def nueva_emergencia():
    if request.method == 'POST':
        d = request.form
        db.execute(
            'INSERT INTO frag_emergencia_stcz (fecha, hora, tipo_emergencia, estado_paciente, observaciones, id_paciente, id_hospital) VALUES (?,?,?,?,?,?,?)',
...
def transferencias():
    rows = db.fetchall(
        "SELECT t.*, p.nombre + ' ' + p.apellido AS paciente, h1.nombre AS origen, h2.nombre AS destino FROM frag_transferencia_stcz t JOIN frag_paciente_stcz p ON t.id_paciente = p.id_paciente JOIN hospital h1 ON t.id_hospital_origen = h1.id_hospital JOIN hospital h2 ON t.id_hospital_destino = h2.id_hospital ORDER BY t.fecha_transferencia DESC"
    )
...
def nueva_transferencia():
    if request.method == 'POST':
        ...
        id_transferencia = db.execute(
            "INSERT INTO frag_transferencia_stcz (fecha_transferencia, motivo, estado, id_paciente, id_hospital_origen, id_hospital_destino) VALUES (?,?,'Pendiente',?,?,?)",
        ...
        data, err = db.lpz_post('/api/transferir_desde', {
            'nodo_origen': 'STCZ', 'id_paciente': int(d['id_paciente']),
...
def buscar_nacional():
    ...
    if q:
        # LPZ es SQL Server: T-SQL dentro del OPENQUERY
        ...
        if not err_ls:
            resultados, metodo = rows_ls, 'LinkedServer'
        else:
...
def emergencia_cruzada(id_paciente):
    replica = db.fetchone('SELECT * FROM historial_replica WHERE id_paciente = ?', (id_paciente,))
...
def api_transferir():
    """Recibe paciente transferido hacia STCZ."""
    d = request.get_json()
    try:
        db.execute_batch(
            "SET IDENTITY_INSERT frag_paciente_stcz ON; "
            "INSERT INTO frag_paciente_stcz (id_paciente, nombre, apellido, ci, fecha_nacimiento, sexo, direccion, telefono, tipo_sangre, alergias, id_hospital) "
            "VALUES (?,?,?,?,?,?,?,?,?,?,?); "
...
def api_paciente(id_paciente):
    p = db.fetchone('SELECT * FROM frag_paciente_stcz WHERE id_paciente = ?', (id_paciente,))
    if p: return jsonify({'success': True, 'data': p, 'nodo': 'STCZ'})
    return jsonify({'success': False, 'error': 'No encontrado'}), 404

@app.route('/api/replica', methods=['POST'])
def api_replica():
    d = request.get_json()
    existing = db.fetchone('SELECT id_replica FROM historial_replica WHERE id_paciente=? AND hospital_origen=?',
                           (d['id_paciente'], d['hospital_origen']))
    if existing:
        db.execute('UPDATE historial_replica SET tipo_sangre=?, alergias=?, enfermedades_cronicas=?, fecha_actualizacion=GETDATE() WHERE id_paciente=? AND hospital_origen=?',
                   (d['tipo_sangre'], d['alergias'], d['enfermedades_cronicas'], d['id_paciente'], d['hospital_origen']))
    else:
        db.execute('INSERT INTO historial_replica (id_paciente, hospital_origen, tipo_sangre, alergias, enfermedades_cronicas) VALUES (?,?,?,?,?)',
...
def api_estado_nodos():
    ...
    nodos = {}
    for key, url, nombre in [('lpz', config.LPZ_URL, 'La Paz (LPZ)'), ('cbba', config.CBBA_URL, 'Cochabamba (CBBA)')]:
        try:
...
def api_health():
    return jsonify({'nodo': 'STCZ', 'estado': 'activo', 'timestamp': datetime.now().isoformat()})
